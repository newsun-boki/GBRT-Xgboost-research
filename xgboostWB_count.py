...
def New_preprocessing(TimeSeries):
   Data=[]
   start_date=datetime(2019, 7, 1,00,00,00) # define start date
   ...
   headers=['value','month','day','hour','day_of_week','day_of_year','week_of_year']
   ...
   #################################################################################################
   # cut training and testing training is 385
   Train=Normalized_Data_df.iloc[0:340,:]
   Train=Train.values
   Train = Train.astype('float32')
   print('Traing length :',len(Train))
   Test=Normalized_Data_df.iloc[:,:]
   Test=Test.values
   ...
   x_testbatches=[]
   y_testbatches=[]
   while start_test+(num_periods_input)<end_test:
        next_test=start_test+num_periods_input
        print(next_test+(num_periods_input))
...
X_Test=[]
Y_Test=[]
x_batches, y_batches,X_Test,Y_Test=New_preprocessing(data)          
...
Unshuffled_All_Testing_Instances = []
#unshuffled_one=============== flatten each training window into Instance =================================
for i in range(0,len(X_Test_Full)):
    ...
    Unshuffled_All_Testing_Instances.append(hold)
#=============== flatten each training window into Instance =================================
print("=============== flatten each training window into Instance =================================")
...
print("=============== flatten each testing window into Instance =================================")
...
#============================== PREDICTION ===============================
print("============================== PREDICTION ===============================")
prediction=multioutput.predict(Unshuffled_All_Testing_Instances)
print('prediction ',prediction.shape)
for i in range(len(prediction)):
    print(float(prediction[i]))
    # print(float(Y_Test_Full[i]))
