...
def New_preprocessing(TimeSeries):
   ...
   Train=Normalized_Data_df.iloc[0:240,:]
   Train=Train.values
   Train = Train.astype('float32')
   print('Traing length :',len(Train))
   Test=Normalized_Data_df.iloc[(240-num_periods_input):360,:]
   Test=Test.values
...
for i in range(len(prediction)):
    for j in range(len(prediction[i])):
        # print(float(prediction[i][j]))
        print(float(Y_Test_Full[i][j]))
